...
public class Geocoder {
	...
	public static Location getLocation(String address) throws IOException {
		try {
			...
			if (Main.addNash.get())
				url.append(URLEncoder.encode(address + ",Nashville,TN",
						ENCODING));
			else
				url.append(URLEncoder.encode(address, ENCODING));
			...
			while ((line = in.readLine()) != null) {
				...
				if (statusCode == 200) {
					...
					if (lat.equals("36.1419303") && lon.equals("-86.8044586")) {
						...
						break;
					} else if (lat.equals("36.1658899")
							&& lon.equals("-86.7844432")) {
						// Got all of Nashville
						break;
					}

					return new Location(address, Double.parseDouble(lat),
							Double.parseDouble(lon));
				} else if (statusCode == 620) {
					// Google is temporarily blocking us
					System.err
							.println("Google is currently blocking GeoCoding from our ip, going to slow ourselves down significantly");
					delay = 3 * 60 * 1000;
				}
			}
			...
			String uriString = "http://api.whitepages.com/find_business/1.0/"
					...
					+ "&zip=37235" + "&api_key=" + whitePagesApiKey;
			...
			// Wait 1/2 second between requests because the geocoding service
			// doesn't allow more than 2 requests/second
			Thread.sleep(500);

			Iterator descendants = doc.getDescendants(new ElementFilter());

			// Iterate through the descendants until we've found a latitude and
			// longitude
			...
			if (lat != null && lon != null) {
				return new Location(address, Double.parseDouble(lat), Double
						.parseDouble(lon));
			} else {
			...
			throw new IOException(
					"Error when parsing geocoding server response");
